...
- `./proxy` - Proxy server binary (root directory)
- `./cdpcli` - CDP client tool for dumping websites (root directory)
- `./chrome.sh` - Script to start Chrome with debugging enabled (root directory)
...
```bash
zig build run -- serve --log_level debug --http_proxy http://127.0.0.1:3000 --insecure_disable_tls_host_verification
```
